...
public class Hash{
	...
	*/
	public int hash(String s){
		...
		for(int i = 0; i < s.length(); i++)
			hashValue += ((int)s.charAt(i)-97)*Math.pow(3, i);
		return Math.abs(hashValue%(TableSize-1));
	}
	...
	*/
	public int insert(String s){
		...
		for(int j = 0; T.getElement(i) != null; j++)
		{
			i += Math.pow(j, 2);
			i = Math.abs(i%(TableSize-1));
		}
		T.setElement(i, s);
	...
	*/
	public int find(String s){
		...
		for(int j = 0; !T.getElement(i).equals(s) && !T.getElement(i).equals(null); j++)
		{
			i += Math.pow(j, 2);
			i = Math.abs(i%(TableSize-1));
			System.out.print(i);
		...
		return (T.getElement(i).equals(s))? i : -1;
